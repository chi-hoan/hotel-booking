...
const FeaturedDestination = () => {
    ...
    <div className='flex flex-col items-center px-6 md:px16 lg:px-24 bg-slate-50 py-20'>
        ...
        <div className='flex flex-grap items-center justify-center gap-6 mt-20'>
            {roomsDummyData.slice(0, 4).map((room, index)=> (
                <HotelCard key={room._id} room={room} index={index}/>
            ))}
        </div>

        <button onClick={()=>{navigate('/rooms'); scrollTo(0, 0)}} className='my-16 px-4 py-2 text-sm font-medium border border-gray-300 rounded bg-white hover:bg-gray-50 transition-all cursur-pointer'>
